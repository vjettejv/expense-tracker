...
// Tháng trước (để so sánh ở Card thống kê)
...
// 2. Pie Chart: Chi tiêu (Có màu danh mục)
...
$pie_exp_labels = []; $pie_exp_data = []; $pie_exp_colors = [];
while($row = $res_pie_exp->fetch_assoc()) { 
    $pie_exp_labels[] = $row['name']; 
    $pie_exp_data[] = $row['total'];
    $pie_exp_colors[] = $row['color'];
}

// 3. Pie Chart: Thu nhập (Có màu danh mục)
...
$pie_inc_labels = []; $pie_inc_data = []; $pie_inc_colors = [];
while($row = $res_pie_inc->fetch_assoc()) { 
    $pie_inc_labels[] = $row['name']; 
    $pie_inc_data[] = $row['total'];
    $pie_inc_colors[] = $row['color'];
}

// 4. Bar Chart: Thu vs Chi (6 tháng) - TÁCH BIỆT THU CHI
$bar_labels = [];
$bar_income = [];
$bar_expense = [];
for ($i = 5; $i >= 0; $i--) {
    // Dùng 'first day of' để tránh lỗi ngày 31 khi lùi tháng
...
<div class="stats-grid">
    <div class="stat-card">
        ...
        <div>
            <div class="stat-sub text-muted" style="text-transform: uppercase;">Tổng Tài Sản</div>
    ...
    <div class="stat-card">
        ...
        <div>
            <div class="stat-sub text-muted" style="text-transform: uppercase;">Thu Nhập</div>
    ...
    <div class="stat-card">
        ...
        <div>
            <div class="stat-sub text-muted" style="text-transform: uppercase;">Chi Tiêu</div>
            <div class="stat-value text-danger">-<?php echo number_format($expense_cur); ?> đ</div>
            <div class="stat-sub <?php echo $growth_class; ?>" style="font-size: 11px;"><?php echo $growth_text; ?> so với tháng trước</div>
        </div>
    </div>
</div>
...
<div style="display: flex; gap: 24px; margin-bottom: 24px; flex-wrap: wrap;">
    <div class="card" style="flex: 1; min-width: 300px;">
        ...
        <div class="chart-wrapper"><canvas id="pieExpense"></canvas></div>
        <?php if(empty($pie_exp_data)) echo "<p style='text-align:center;color:#999;font-size:13px;'>Chưa có dữ liệu.</p>"; ?>
    </div>
    <div class="card" style="flex: 1; min-width: 300px;">
        <h3 style="margin-top: 0; text-align: center;">💰 Cơ cấu Thu Nhập</h3>
        <div class="chart-wrapper"><canvas id="pieIncome"></canvas></div>
        <?php if(empty($pie_inc_data)) echo "<p style='text-align:center;color:#999;font-size:13px;'>Chưa có dữ liệu.</p>"; ?>
    </div>
</div>
...
<div style="display: flex; gap: 24px; flex-wrap: wrap;">
    ...
    <div class="card" style="flex: 1; min-width: 400px;">
        <h3 style="margin-top: 0;">📊 So sánh Thu - Chi (6 tháng)</h3>
        <div class="chart-wrapper"><canvas id="barHistory"></canvas></div>
    </div>
    <!-- Biểu đồ đường -->
    <div class="card" style="flex: 1; min-width: 400px;">
        <h3 style="margin-top: 0;">📈 Xu hướng chi tiêu theo ngày</h3>
        <div class="chart-wrapper"><canvas id="lineTrend"></canvas></div>
    </div>
</div>
...
<script>
    ...
    new Chart(document.getElementById('pieExpense'), {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: <?php echo json_encode($pie_exp_colors); ?>,
                borderWidth: 0
            }]
        },
    ...
    new Chart(document.getElementById('pieIncome'), {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: <?php echo json_encode($pie_inc_colors); ?>,
                borderWidth: 0
            }]
        },
        options: { maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { boxWidth: 12 } } } }
    ...
    // 3. BAR CHART: THU VS CHI (Quan trọng: Tách riêng 2 cột)
            ...
            datasets: [
                {
                    ...
                    backgroundColor: '#10b981', // Màu Xanh cố định cho Thu
                    borderRadius: 4,
                    barPercentage: 0.6,
                ...
                },
                {
                    ...
                    backgroundColor: '#ef4444', // Màu Đỏ cố định cho Chi
                    borderRadius: 4,
                    barPercentage: 0.6,
                    categoryPercentage: 0.8
                }
