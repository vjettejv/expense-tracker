...
if (isset($_SESSION['user_id'])) {
    if ($_SESSION['role'] == 'admin') {
        ...
        header("Location: D:/xampp/htdocs/expense-tracker/index.php");
    }
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username) || empty($password)) {
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            $error_msg = "Lỗi hệ thống, thử lại sau.";
        }
    }
    $conn->close();
}
...
<style>
    * {
    ...
    body {
        background-color: #fafafa; /* Màu nền xám nhạt giống Instagram */
    }

    #main {
        margin: 48px auto; /* Tăng khoảng cách trên cho thoáng */
        display: flex;
        width: 350px;
        align-items: center;
    ...
    .imgLogo {
        width: 175px;
        height: auto; /* Để auto cho đỡ bị méo ảnh */
    }
    ...
    .form-control {
        ...
        width: 268px; /* Chỉnh lại chút cho cân đối */
        height: 38px;
        font-size: 12px;
        border: 1px solid #dbdbdb;
        border-radius: 3px;
...
<script>
    ...
    function showToast(message, type = 'error') {
        ...
        if (type === 'success') {
            ...
            noti.style.backgroundColor = '#ed4956'; // Màu đỏ Instagram
        }
    ...
    // 1. Kiểm tra lỗi từ PHP (Server trả về)
    <?php if (!empty($error_msg)): ?>
        showToast("<?php echo $error_msg; ?>", "error");
    <?php endif; ?>

    // 2. Kiểm tra trước khi gửi form (Client side)
    document.getElementById("formmain").addEventListener("submit", function (event) {
        const username = document.getElementById("username").value.trim();
        const password = document.getElementById("password").value.trim();

        if (username === "" || password === "") {
            event.preventDefault(); // Chặn gửi nếu rỗng
            showToast('Vui lòng nhập đầy đủ thông tin!', 'error');
        }
        // Nếu không rỗng thì để form tự submit lên PHP xử lý
    });
</script>
